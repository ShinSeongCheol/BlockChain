...
#if defined(NDEBUG)
# error "Ngncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2202fddefaef5f18e62be0eefd5830d4ca15f77b948e92389a008626c4042705");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ngncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ngncoin Signed Message:\n";
...
{
    // Ngncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1840000); // Ngncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Ngncoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Ngncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ngncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ngncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd248f06eead18b54c7f1503b20197ba2040f7b71502a6153601c374b9ce42c13");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "My first Ngn Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1633076516;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085129608;

        if (fTestNet)
        {
            block.nTime    = 1633076520;
            block.nNonce   = 386784200;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x069f2a82087de0654eebbec55741752cc9b67d34264354c755a2c1550660c6a7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // Ngncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NgncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NgncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NgncoinMiner(CWallet *pwallet)
{
    printf("NgncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ngncoin-miner");
    ...
    try { loop {
        ...
        printf("Running NgncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NgncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NgncoinMiner, pwallet));
}
